...
CACHE_TTL_SECONDS = 15 * 60  # 15 dk — aynı konum için gereksiz çağrı yapma

# Basit in-memory cache: {(lat, lon, kind): (timestamp, data)}
...
class CurrentConditions:
    ...
    humidity_pct: float
    uv_index: float
    pollen_level: str  # "Low" | "Moderate" | "High" (bkz. _classify_pollen)
...
def _classify_pollen(grass: Optional[float], birch: Optional[float]) -> str:
    """Open-Meteo pollen verisi çoğunlukla Avrupa için mevcut.
    Yoksa (None) 'Unavailable' döndür — UI bunu 'Moderate' gibi
    yanlış göstermesin."""
...
def get_current_conditions(lat: float, lon: float) -> CurrentConditions:
    """Şu anki hava + hava kalitesi verisini döndürür.
    dashboard.html, environment.html ve ai.html'in üstteki
    'Environmental Feed' kartları için kullanılır."""
...
def get_history(lat: float, lon: float, days: int = 7) -> list[dict]:
    """Son N günün AQI / sıcaklık / nem verisini döndürür.
    environment.html'deki 7-Day Chart ve patterndetector.py'nin
    baseline hesaplaması için kullanılır."""
    ...
    # Saatlik AQI'yi günlük ortalamaya indir
